...
export default withSession(async (req, res) => {
  ...
  if (user) {
    ...
    if (checkExpired(user?.data?.login[aTIndex])) {
      // Get new access/auth token
      const newAccessToken = (
        await refreshAuthToken(user?.data?.login[rtIndex])
      )[aTIndex];

      // Remove old access/auth token and store in cookie
      delete user?.data?.login[aTIndex];
      const newUser = { data: { login: { aTIndex: newAccessToken }, ...user } };
